const SHOP_DATA = [
  {
    id: 1,
    title: 'All Occasions',
    route: 'all-occasions',
    items: [
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003850/arrangements/phaelenopsis-orchid-garden_seodpx.png',
        id: 5,
        name: 'Orchid Garden',
        shortDescription:
          'A perfect bouquet to give to anybody to express your love, care, congratulations, anniversary, birthday, thank you, miss you, get well etc.',
        description:
          'A perfect bouquet to give to anybody to express your love, care, congratulations, anniversary, birthday, thank you, miss you, get well etc. The bouquet made as one side view and a bit in long stems they are mixed spring flowers variety of mum, ]chrysanthemums, rose, lisianthus, tulips, spray rose and some greenish with a variety of foliages to make it so elegant in your hand or when it’s placed in the glass water vase (included) on table as a home decor.',
        pricingText: 50,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003850/arrangements/neon-light-city-design_jgzsyq.png',
        id: 6,
        name: 'Neon Light City',
        shortDescription:
          'The arrangement I make specially for graduation, bridesmaids or birthday girls, the compact size and the round shape makes this bouquet so adorable. ',
        description:
          'The arrangement I make specially for graduation, bridesmaids or birthday girls, the compact size and the round shape makes this bouquet so adorable. This bouquet also can be placed in the vase after the event is done and turn into a home decor or even centerpiece. Mixed spring flowers and use mostly bright and happy colors that give a young look, energetic and casual.',
        pricingText: 42,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/w_500,h_550/v1656003850/arrangements/metropolitan-design_fashox.png',
        id: 7,
        name: 'Metropolitan',
        shortDescription: 'A perfect bouquet to gift for any occasion.',
        description:
          'A perfect bouquet to gift for any occasion. The bouquet made as one side view and a bit in long stems they are mixed spring flowers variety of mum, chrysanthemums, roses, lisianthus, tulips, spray roses and some greenish with a variety of foliages to make it so elegant in your hand or when it’s placed in a ceramic water vase (included) on table as a home decor. ',
        pricingText: 49.5,
      },

      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003850/arrangements/cosmoplolitan-design-alt_pmwds2.png',
        id: 19,
        name: 'Chic Centerpiece',
        shortDescription:
          'For graduations, bridesmaids or birthday girls. Called chic for its petite shape.',
        description:
          'For graduations, bridesmaids or birthday girls. Called chic for its petite shape. This bouquet also can be placed in the vase after the event  is done and turn into a home decor or even as a centerpiece. Mixed spring flowers and use mostly bright and happy colors. So comfortable to hold in your hand, modern and classy.',
        pricingText: 42,
      },
    ],
  },
  {
    id: 2,
    title: 'Bridal',
    route: 'bridal',
    items: [
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003850/arrangements/citys-tulips-design_nqwe7c.png',
        id: 20,
        name: "City's Tulips",
        shortDescription:
          'A gorgeous tulip bouquet with chrysanthemum and a variety of mums and foliages. ',
        description:
          'A gorgeous tulip bouquet with chrysanthemum and a variety of mums and foliages. Accesorized with curly willows to make this bouquet stunning. Available in one or two colors of your choice and in bunches of 18", 24 or 36 tulips.',
        pricingText: 89,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003846/arrangements/asymmetric-centerpiece-design_anphoq.png',
        id: 21,
        name: 'Asymmetric Centerpiece',
        pricingText: 60,
        shortDescription:
          'The dramatic curve of this asymmetrical arrangement makes for an excellent centerpiece.',
        description:
          'The dramatic curve of this asymmetrical arrangement makes for an excellent centerpiece. Its unusual, mystifying shape give an elegant look to your dining table. Predominantly made of roses, along with carnations, hypericum, delphinium, eucalyptus, evangarium, curly willows and foliage.',
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1656003848/arrangements/citys-rose-design_dqwdgr.png',
        id: 22,
        name: "City's Rose",
        shortDescription:
          'A gorgeous roses  bouquet with huckleberry, alstroemeria and foliage accessories with curly willows to make this roses arrangement stunning.',
        description:
          'A gorgeous roses  bouquet with huckleberry, alstroemeria and foliage accessories with curly willows to make this roses arrangement stunning. Available in one or two colors and in bunches of 18", 24, or 36 roses.',
        pricingText: 25,
      },
    ],
  },
  {
    id: 3,
    title: 'Holidays',
    route: 'holidays',
    items: [
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057740/arrangements/city-combo-three-in-one_no0ozp.png',
        id: 24,
        name: "City's Three-In-One Combo",
        shortDescription: 'No Description Available',
        description: 'No Description Available',
        pricingText: 69,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1658965613/arrangements/small-dried-flowers_qvfepx.png',
        id: 25,
        name: 'Small Dried Flowers',
        shortDescription: 'No Description Available',
        description: 'No Description Available',
        pricingText: 12,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057742/arrangements/metropolitan-tropical-2_cgv8p6.png',
        id: 26,
        name: 'Metropolitan Tropical Design',
        shortDescription: 'No Description Available',
        description: 'No Description Available',
        pricingText: 35,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1659375224/arrangements/cosmo-three-in-one_naynvl.png',
        id: 27,
        name: 'Cosmopolitan Three-In-One Combo',
        shortDescription:
          'A set of combination three flower designs, your choice of vertical, asymmetrical or center price design or can be mixed and matched to collaborate and contrast each other.',
        description:
          'A set of combination three flower designs, your choice of vertical, asymmetrical or center price design or can be mixed and matched to collaborate and contrast each other. Your option to pick of 3 total designs option (give us the number of each choices of could all of them in the same design',
        pricingText: 120,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057754/arrangements/massive-metropolitan-2_n550gi.png',
        id: 28,
        name: 'Massive Metropolitan Design',
        shortDescription:
          'A pretty big size design for a ballroom, house staging , hallways or welcoming with a combination of tropical and mixed spring flowers material will give a glamor’s look for your events.',
        description:
          'A pretty big size design for a ballroom, house staging , hallways or welcoming with a combination of tropical and mixed spring flowers material will give a glamor’s look for your events. ',
        pricingText: 129,
      },
    ],
  },
  {
    id: 4,
    title: 'Best Wishes',
    route: '/shop/well-wishes',
    items: [
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057743/arrangements/medium-dried-flowers_qifekx.png',
        id: 41,
        name: 'Medium Dried Flowers',
        shortDescription: 'No Description Availeble',
        description: 'No Description Availeble',
        pricingText: 60,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057747/arrangements/massive-metropolitan_jyj0ht.png',
        id: 42,
        name: 'Massive Metropolitan',
        shortDescription: 'No Description Availeble',
        description: 'No Description Available',
        pricingText: 60,
      },
    ],
  },
  {
    id: 5,
    title: 'Deals',
    route: 'deals',
    items: [
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057753/arrangements/metropolitan-in-house_vcmktt.png',
        id: 51,
        name: 'Metropolitan In-House',
        shortDescription: 'No Description Availeble',
        description: 'No Description Available',
        pricingText: 60,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057756/arrangements/metropolitan-bouquet_g9i0pm.png',
        id: 52,
        name: 'Metropolitan Bouquet',
        shortDescription: 'No Description Availeble',
        description: 'No Description Available',
        pricingText: 60,
      },

      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057742/arrangements/metropolitan-tropical-2_cgv8p6.png',
        id: 57,
        name: 'Tropical Metropolitan Design',
        shortDescription:
          'The flower arrangement is symbolic of the expression of losing someone you love.',
        description:
          'The flower arrangement is symbolic of the expression of losing someone you love. I provide a variety of funeral floral arrangements as you say goodbye for the last time.   Fram full or half casket spray, sympathy bouquets and wreaths.',
        pricingText: 100,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057747/arrangements/metropolitan-neon-lights_ggmncg.png',
        id: 58,
        name: 'Neon Lights Metropolitan',
        shortDescription:
          'A beautiful medium size  design with light on it to make this arrangement such a gorgeous to look at.',
        description:
          'A beautiful medium size  design with light on it to make this arrangement such a gorgeous to look at. Mixed spring flowers with Ginger &  Kale as focal and it’s accessorized with butterflies (optional). Perfect for a wedding reception, bridal shower, baby shower or any important events. ',
        pricingText: 129,
      },
      {
        imageUrl:
          'https://res.cloudinary.com/dgdnpkfun/image/upload/v1657057743/arrangements/metropolitan-funeral-package-2_cq4qjg.png',
        id: 59,
        name: 'Metropolitan Sympathy & Funeral Design',
        shortDescription:
          'The flower arrangement is symbolic of the expression of losing someone you love.',
        description:
          'The flower arrangement is symbolic of the expression of losing someone you love. I provide a variety of funeral floral arrangements as you say goodbye for the last time.   Fram full or half casket spray, sympathy bouquets and wreaths.',
        pricingText: 100,
      },
    ],
  },
];

export default SHOP_DATA;
